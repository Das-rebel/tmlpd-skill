...
exports.executeEnsemble = executeEnsemble;
exports.mergeComplementary = mergeComplementary;
 ...
 */
async function executeEnsemble(query, systemPrompt, context, providerExecutors, config = {}) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    const start = Date.now();
    const perProvider = {};
    // Step 1: Fire ALL providers in parallel
    const results = await Promise.allSettled(cfg.providers.map(async (name) => {
        const pStart = Date.now();
        try {
            const executor = providerExecutors[name];
            if (!executor)
                throw new Error(`No executor for ${name}`);
    ...
    for (const name of cfg.providers) {
        ...
        if (cfg.scoringWeights.lengthPenalty > 0) {
            ...
            if (len < 50)
                ...
                score += cfg.scoringWeights.lengthPenalty * 10;
        }
 ...
 */
function mergeComplementary(results, maxLength = 4000) {
 ...
 */
function recordFeedback(winner, wasHelpful, history) {
    if (!history[winner])
        history[winner] = { good: 0, bad: 0 };
    if (wasHelpful)
        history[winner].good++;
    else
        history[winner].bad++;
...
//# sourceMappingURL=ensembleVoting.js.map